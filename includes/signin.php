...
if (isset($_POST['signin'])) {
    ...
    $sql = "SELECT id, FullName, EmailId, fname, lname, Password, Status, session_id FROM tblusers WHERE EmailId=:email AND Status = :stat";
    ...
    if ($query->rowCount() > 0) {
        if ($user['Status'] == 2) {
            // Check if the user already has an active session in another browser
            if (isset($user['session_id']) && $user['session_id'] != session_id()) {
                echo "<script>
                    Swal.fire({
                        title: 'Error!',
            ...
            if (password_verify($password, $user['Password'])) {
                // Store the session ID to ensure only one active session per user
                $session_id = session_id();

                // Update the session_id for the user in the database
                $sql_update = "UPDATE tblusers SET session_id = :session_id WHERE EmailId = :email";
